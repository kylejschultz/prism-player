...
After both release installers are attached, the same workflow posts a Discord release card, pings the Prism release role, and provides direct macOS and Windows download buttons plus the GitHub release link. Set the `DISCORD_RELEASE_WEBHOOK_URL` repository secret to the Discord channel webhook before publishing a release.
